        ...
        <View style={[styles.frame, wide && styles.frameWide]}>
          ...
          <Animated.View entering={FadeInDown.duration(340)} style={[styles.content, wide && styles.contentWide]}>
            <View style={[styles.imagePanel, wide && styles.imagePanelWide]}>
              <View style={styles.mainImageWrapper}>{(() => { const allImages = [item.image_url, ...galleryImages.map((g: any) => g.image_url)].filter(Boolean); const src = allImages[activeImageIndex]; return src ? <Image source={{ uri: src }} style={styles.image} /> : <Feather name="box" size={88} color={Design.color.inkMuted} />; })()}</View>
              {(() => { const allImages = [item.image_url, ...galleryImages.map((g: any) => g.image_url)].filter(Boolean); return allImages.length > 1 ? <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.thumbnailStrip}>{allImages.map((url: string, i: number) => <Pressable key={url + i} onPress={() => setActiveImageIndex(i)} style={[styles.thumbnailItem, i === activeImageIndex && styles.thumbnailActive]}><Image source={{ uri: url }} style={styles.thumbnailImage} /></Pressable>)}</ScrollView> : null; })()}
            </View>
